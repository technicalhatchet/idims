...
function ReasoningAccordion({ section, variant, defaultOpen = false }) {
  ...
  return (
    <div
      className={`rounded-lg border ${
        isMobile ? 'border-white/10 bg-[#0a101c]' : 'border-gray-200 dark:border-gray-700'
      }`}
    >
      <button
        type="button"
...
export default function SolomonReasoningSheet({
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col justify-end" role="dialog" aria-modal="true" aria-label="Diagnostic reasoning">
      ...
      />
      <div
        className={`relative mx-auto w-full max-w-lg max-h-[92vh] flex flex-col rounded-t-2xl border-t border-white/10 shadow-2xl ${
          isMobile ? 'bg-[#0A0F1E]' : 'bg-white dark:bg-gray-900'
        }`}
        style={{ marginTop: 'auto', marginBottom: 0 }}
      >
        <div className="flex items-center justify-between gap-3 px-4 py-3 border-b border-white/10 shrink-0">
          <p className="text-[10px] uppercase tracking-[0.2em] text-cyan-400/90 font-medium">
        ...
        <div className="flex-1 overflow-y-auto px-4 py-4 space-y-4 pb-[max(1rem,env(safe-area-inset-bottom))]">
          {lead ? (
            <div className="rounded-xl border border-emerald-500/20 bg-emerald-500/[0.06] px-3 py-3">
              <p className="text-[10px] uppercase tracking-wide text-gray-500">Current leading hypothesis</p>
          ...
          <div>
            <p className="text-[10px] uppercase tracking-wide text-cyan-400/80 mb-2">Why we believe this</p>
          ...
          <div className="space-y-2">
            {accordionSections.map((section) => (
              <ReasoningAccordion key={section.id} section={section} variant={variant} />
            ))}
          </div>

          {preview ? (
            <div className="rounded-xl border border-cyan-500/20 bg-cyan-500/[0.05] px-3 py-3">
              <p className="text-[10px] uppercase tracking-wide text-cyan-400/80">Next test preview</p>
