      ...
      titleplate={
        <div className="flex items-center justify-between gap-3">
          ...
          <div className="flex-1 min-w-0 text-center">
            <p className="tx-view-hud-orbitron text-[8px] uppercase tracking-[0.2em] text-cyan-300/95 mb-1">Operative Profile</p>
            <h1 className="tx-view-hud-orbitron text-base font-black uppercase tracking-[0.08em] text-white truncate">{displayName}</h1>
          </div>
          <Link href={`/technicians/${id}/txmobile_edit`} className="text-cyan-400" data-hud-card>
            <FaEdit size={18} />
          </Link>
        </div>
      }
    >
      <div className="rounded-lg mb-4 overflow-hidden border border-cyan-400/30 bg-[rgba(13,21,37,0.85)]" data-hud-card>
        <button
          type="button"
        ...
        {activeSection === 'details' && (
          <div className="px-4 pb-4 space-y-3 border-t border-cyan-900/30">
            ...
            <div>
              ...
              <span className="inline-block px-2 py-1 text-xs uppercase tracking-wide font-medium rounded bg-cyan-400/15 text-cyan-400">
      ...
      <Link
        ...
        className="flex items-center justify-center gap-2 w-full px-4 py-3 rounded-lg text-sm font-bold uppercase tracking-wide text-cyan-400 border border-cyan-400/40 bg-cyan-400/10"
        data-hud-card
      >
        <FaChartLine />
        View Performance
