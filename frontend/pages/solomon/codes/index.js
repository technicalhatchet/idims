...
import {
  SOLOMON_CODES_RESULT_LINK_CLASS,
  SOLOMON_CODES_SEARCH_BUTTON_CLASS,
  ...
  SOLOMON_REFERENCE_ICON_SHELL_CLASS,
} from '../../../components/solomon/solomonListPageUi';

function CodeResultRow({ item }) {
  return (
    <li>
      <Link href={`/solomon/codes/${item.id}`} className={SOLOMON_CODES_RESULT_LINK_CLASS}>
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0">
            <p className={SOLOMON_REFERENCE_CODE_TEXT_CLASS}>{item.code}</p>
            <p className="text-sm text-[var(--solomon-text-primary)] mt-1 line-clamp-2">{item.meaning}</p>
            <p className="text-[11px] text-[var(--solomon-text-muted)] mt-2">
              {item.manufacturer} · {formatDmaSubtype(item.equipment_subtype)}
            </p>
          </div>
          <span className="text-xs text-[var(--solomon-text-muted)] shrink-0 pt-0.5" aria-hidden>→</span>
        </div>
      </Link>
    </li>
  );
...
export default function SolomonErrorCodesPage() {
  ...
  const referenceEyebrow = (
    <div className="flex items-center gap-2 mb-2">
      <span className={SOLOMON_REFERENCE_ICON_SHELL_CLASS}>
        <FaHashtag size={14} aria-hidden />
      </span>
      <p className={SOLOMON_REFERENCE_EYEBROW_CLASS}>Reference</p>
    </div>
  );
  ...
  return (
    <SolomonListPage
      ...
      title="Error codes"
      description="Manufacturer fault codes with meaning, common causes, and recommended fixes."
      back="arrow"
      backHref="/solomon"
      backLabel="Back to Solomon home"
      eyebrow={referenceEyebrow}
    >
