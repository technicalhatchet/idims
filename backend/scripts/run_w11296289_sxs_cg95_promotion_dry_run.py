#!/usr/bin/env python3
"""CG-9.5 WP2 — Whirlpool W11296289 SxS promotion dry-run (fail-closed, no publish)."""
...
from whirlpool_w11296289_sxs_cg95_gate_publish import (
...
TABLE_PATH = CALIBRATION_DIR / "W11296289_SXS_overlay_mapping_table_v1.json"
LEDGER_PATH = CALIBRATION_DIR / "W11296289_SXS_gate_decision_ledger_v1.json"
...
def main() -> int:
    print("==> CG-9.5 Whirlpool W11296289 SxS WP2 promotion dry-run")

    if not TABLE_PATH.is_file() or not LEDGER_PATH.is_file():
        print("FAIL: run apply_w11296289_sxs_cg95_human_gate.py first", file=sys.stderr)
        return 1
    ...
    checks = {
        ...
        "conditionalBindingsSix": acct.get("conditionalBindings", 0) == 6,
        ...
        "platformIdAuthoritative": family.get("platformId") == "whirlpool_sxs_w11296289",
        ...
        "doorSwitchInProcedureBindings": any(
            ...
            if str(b.get("procedureId", "")).startswith("w11296289-test-2")
        ),
        ...
        "noJazzProcedureBindings": not any(
            str(b.get("procedureId", "")).startswith("w10322959-")
            for b in (family.get("procedureBindings") or [])
        ),
    ...
    plan_path = CALIBRATION_DIR / "publication_plan_W11296289_SXS_CG95.json"
