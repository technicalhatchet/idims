#!/usr/bin/env python3
"""CG-9.5 WP4 — Midea RSS SxS promotion dry-run (fail-closed, no publish)."""
...
from midea_rss_cg95_gate_publish import (
...
TABLE_PATH = CALIBRATION_DIR / "MIDEA_RSS_SXS_overlay_mapping_table_v1.json"
LEDGER_PATH = CALIBRATION_DIR / "MIDEA_RSS_SXS_gate_decision_ledger_v1.json"
...
def main() -> int:
    print("==> CG-9.5 Midea RSS SxS WP4 promotion dry-run")

    if not TABLE_PATH.is_file() or not LEDGER_PATH.is_file():
        print("FAIL: run apply_midea_rss_cg95_human_gate.py first", file=sys.stderr)
        return 1
    ...
    lg_sxs_hash_before = file_sha256(LG_SXS_OVERLAY_PATH)
    ...
    lg_sxs_hash_after = file_sha256(LG_SXS_OVERLAY_PATH)
    ...
    checks = {
        ...
        "canonicalInheritancesThree": acct.get("canonicalInheritances", 0) == 3,
        "conditionalBindingsFive": acct.get("conditionalBindings", 0) == 5,
        "overlayLearningTen": acct.get("overlayLearningEvents", 0) == 10,
        "intentionalAbstentionsFive": acct.get("intentionalAbstentions", 0) == 5,
        ...
        "lgSxsOverlayByteStable": lg_sxs_hash_before == lg_sxs_hash_after,
        ...
        "platformIdAuthoritative": family.get("platformId") == "midea_rss",
        "vfdCompressorConditionalBound": any(
            b.get("conditionalConceptId") == "compressor"
            and b.get("instanceScope") == "vfd_mediated"
        ...
        "doorSwitchNotInProcedureBindings": not any(
            ...
            if str(b.get("procedureId", "")).startswith("midearss-")
        ),
        ...
        "noLgSxsProcedureBindings": not any(
            str(b.get("procedureId", "")).startswith("lgsxs-")
            for b in (family.get("procedureBindings") or [])
        ),
        ...
        "noLinearCompressorPlatformComponent": not any(
            c.get("id") in {"linear_compressor", "io_expander", "compressor_relay"}
    ...
    plan_path = CALIBRATION_DIR / "publication_plan_MIDEA_RSS_SXS_CG95.json"
