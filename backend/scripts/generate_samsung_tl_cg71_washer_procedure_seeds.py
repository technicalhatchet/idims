#!/usr/bin/env python3
"""Generate Samsung TL CG71 washer (WA50R / WA51DG / WF45A) procedure seed JSON files."""
...
OUT = ROOT / "frontend" / "components" / "diagnostics" / "procedures" / "seed" / "samsung_tl_washer_cg71"
...
PROCEDURES = [
    proc(
        "samsungtlcg71-water-level-sensor",
    ...
    proc(
        "samsungtlcg71-detergent-drawer",
        "§5-2: Detergent drawer (SDC)",
        "5-2",
        "Detergent Drawer SDC",
        [32, 35],
        ["dispenser"],
        ["SDC", "dispenser_check"],
        [
            visual(
                "drawer_closed",
                2,
                "Detergent drawer fully closed",
                "Close the detergent drawer until it clicks. SDC displays when the drawer is not completely closed before start.",
                cp_yes_no(
                    "drawer_ok",
                    "drawer_verified",
                    "close_drawer",
                    "close_drawer_out",
                    "Close drawer until click and clear detergent buildup in track.",
                ),
            ),
            outcome("close_drawer_out", 3, "Close detergent drawer", "Seat drawer until click; remove debris blocking closure."),
            outcome("drawer_verified", 4, "Drawer OK", "Detergent drawer closes and latches — retest cycle."),
        ],
    ),
]
...
def smart_install_bundle() -> dict:
    return {
        "id": "samsungtlcg71-smart-install-entry",
        "version": "1.0.0",
        "platformId": "samsung_tl_washer_cg71",
        "manualId": SOURCE["manualId"],
        "title": "Samsung TL CG71 — Smart Install entry",
...
def manual_check_bundle() -> dict:
    return {
        "id": "samsungtlcg71-manual-check-mode",
        "version": "1.0.0",
        "platformId": "samsung_tl_washer_cg71",
        "manualId": SOURCE["manualId"],
        "title": "Samsung TL CG71 — Manual check mode",
        "modeKind": "load_test",
        "uiVariants": ["any"],
...
def diagnostic_code_bundle() -> dict:
    return {
        "id": "samsungtlcg71-diagnostic-code-check",
        "version": "1.0.0",
        "platformId": "samsung_tl_washer_cg71",
        "manualId": SOURCE["manualId"],
        "title": "Samsung TL CG71 — Diagnostic code display",
...
def write_catalog() -> None:
    catalog = {
        ...
        "platformId": "samsung_tl_washer_cg71",
        "templateId": "washer",
        "label": "Samsung TL washer CG71 (WA55CG71, WA54CG, WA55A7)",
...
def main() -> None:
    ...
    write_catalog()
    for script in (
        "attach_samsung_tl_cg71_washer_diagnostic_effects.py",
        "attach_samsung_tl_cg71_washer_service_modes.py",
        "attach_samsung_tl_cg71_washer_procedure_diagrams.py",
    ):
