#!/usr/bin/env python3
"""CG-8 WP2 — Samsung RF23BB promotion dry-run (fail-closed, no publish)."""
...
from samsung_rf23bb_fd_cg8_gate_publish import (
...
TABLE_PATH = CALIBRATION_DIR / "SAMSUNG_RF23BB_FD_overlay_mapping_table_v1.json"
LEDGER_PATH = CALIBRATION_DIR / "SAMSUNG_RF23BB_FD_gate_decision_ledger_v1.json"
CANONICAL_PATH = CANONICAL_DIR / "french_door_refrigerator.json"
OVERLAY_PATH = MANUFACTURER_OVERLAYS_DIR / OVERLAY_FILE
JAZZ_OVERLAY = JAZZ_OVERLAY_PATH
...
def main() -> int:
    print("==> CG-8 Samsung RF23BB WP2 promotion dry-run")

    if not TABLE_PATH.is_file() or not LEDGER_PATH.is_file():
        print("FAIL: run apply_samsung_rf23bb_fd_cg8_human_gate.py first", file=sys.stderr)
        return 1
    ...
    jazz_hash_before = file_sha256(JAZZ_OVERLAY)
    ...
    jazz_hash_after = file_sha256(JAZZ_OVERLAY)
    overlay_blob = json.dumps(overlay_after).lower()
    ...
    checks = {
        ...
        "noJazzLeakInSamsungOverlay": all(term not in overlay_blob for term in JAZZ_LEAK_TERMS),
    ...
    plan_path = CALIBRATION_DIR / "publication_plan_SAMSUNG_RF23BB_FD_CG8.json"
