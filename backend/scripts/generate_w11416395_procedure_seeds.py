#!/usr/bin/env python3
"""Generate W11416395 (Maytag MVW6200 PSC top-load washer) procedure seed JSON files."""
...
OUT = ROOT / "frontend" / "components" / "diagnostics" / "procedures" / "seed" / "whirlpool_mvw6200"
BUNDLE_OUT = OUT / "bundles"
PLATFORM = "whirlpool_mvw6200"

SOURCE = {
    "manualId": "W11416395",
    "manualTitle": "Maytag 4.8 cu ft Top Load Washer (MVW6200, W11416395B)",
    "extractedTextFile": "backend/docs/manuals/service-manual-w11416395-revb mvw6200-extracted.txt",
...
PROCEDURES = [
    ...
    proc(
        "w11416395-test-03-drive-system",
        ...
        [
            ...
            outcome("drive_deferred", 7, "Run TEST #3a or #3b", "Proceed to w11416395-test-03a-shifter or w11416395-test-03b-motor per fault code."),
        ],
    ),
    proc(
        "w11416395-test-03a-shifter",
    ...
    proc(
        ...
        [
            visual(
                ...
                "In Service Diagnostic, press Key 1 for Button Activation & Encoder test. Do keys toggle LED groups and encoder rotate status LEDs?",
                [
            ...
            instr("retest_hmi", 6, "Retest HMI in Service mode", "Reassemble, restore power, enter Service Diagnostic, rerun Button Activation test.", "hmi_verified"),
    ...
    proc(
        "w11416395-test-05-temp-thermistor",
        ...
        [
            instr("therm_sensor_feedback", 2, "Inlet Thermistor Sensor Feedback", "In Service Diagnostic Sensor Feedback, run Inlet Thermistor test. Cold valve opens (temp drops), then hot (temp rises).", "disconnect_j8_therm"),
            instr("disconnect_j8_therm", 3, "Disconnect J8", "If sensor feedback fails: power off, remove J8 from ACU.", "therm_ohms"),
            meas(
                "therm_ohms",
                4,
                "Inlet thermistor J8-8 to J8-9",
                "Measure resistance at ambient. Compare to OEM R/T table (~50 kΩ @ 77°F / 25°C).",
                "whirlpoolMvw6200WasherInletThermistorOhms",
                "J8",
                "8 & 9",
                [
...
PROCEDURE_FILES = [
    "w11416395-test-01-acu-power.json",
    "w11416395-test-02-valves.json",
    "w11416395-test-03-drive-system.json",
    "w11416395-test-03a-shifter.json",
    "w11416395-test-03b-motor.json",
    "w11416395-test-04-hmi.json",
    "w11416395-test-05-temp-thermistor.json",
    "w11416395-test-06-water-level.json",
    "w11416395-test-07-drain-pump.json",
    "w11416395-test-08-lid-lock.json",
]


def service_diagnostic_entry_bundle() -> dict:
    return {
        "id": "w11416395-service-diagnostic-entry",
        "version": "1.0.0",
        "platformId": PLATFORM,
        "manualId": "W11416395",
        "title": "W11416395 — Service Diagnostic mode entry",
...
def service_test_mode_bundle() -> dict:
    return {
        "id": "w11416395-service-test-mode",
        "version": "1.0.0",
        "platformId": PLATFORM,
        "manualId": "W11416395",
        "title": "W11416395 — Service Test Mode",
        "modeKind": "load_test",
        "uiVariants": ["console"],
        "description": "Enter Service Test Mode via Key 2 then START; auto-sequence runs valves, drain, wash, spin.",
        ...
        "steps": [
            {
                ...
                "body": (
                    ...
                    "All LEDs turn on indicating successful entry. Press START again to begin automatic test sequence: "
                    "water valves → drain pump → wash (CW/CCW agitate) → spin (140/300/500 rpm). "
                    "Key 1 repeats prior step; Key 2 skips. Door must be closed. POWER exits to standby."
                ),
                "sourceExcerpt": "To enter Service test mode, press and release Key 2... then press and release the START button.",
                "requiresInput": False,
                "defaultNextStepId": "@continue",
            }
        ],
...
BUNDLE_FILES = [
    "w11416395-service-diagnostic-entry.json",
    "w11416395-service-test-mode.json",
]
...
def write_catalog() -> None:
    catalog = {
        ...
        "platformId": PLATFORM,
        "templateId": "washer",
        "label": "Maytag 4.8 cu ft PSC top-load washer (MVW6200)",
        "notes": "TEST #1–8 (+3a shifter / 3b motor). Service Diagnostic: Key 1/2/3 × 3 within 8 sec; Service Test via Key 2 + START.",
        "plannedProcedures": [
            {
                "id": proc_data["id"],
...
def write_readme() -> None:
    readme = """# whirlpool_mvw6200 — W11416395 procedure seeds
