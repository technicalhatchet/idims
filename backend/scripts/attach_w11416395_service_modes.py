#!/usr/bin/env python3
"""Attach W11416395 service-mode bundles to procedure seeds."""
...
BRIDGE_STEP = {
    ...
    "body": "You should now be in Service Diagnostic mode. Press Key 2 then START to enter Service Test Mode for load/function commands.",
    "sourceExcerpt": "Service Diagnostic mode must be active before Service Test Mode (Key 2 + START).",
    "requiresInput": False,
}

SPLIT_SERVICE_TEST_PROCEDURES: dict[str, tuple[str, str]] = {
    "w11416395-test-02-valves.json": ("valve_live_precheck", "live_test_valves"),
    "w11416395-test-07-drain-pump.json": ("reconnect_pump_power", "live_test_drain_pump"),
}

SINGLE_ENTRY_PROCEDURES: dict[str, tuple[str, str]] = {
    "w11416395-test-03-drive-system.json": ("safety_power_off", "enter_service_diag"),
    "w11416395-test-03a-shifter.json": ("safety_power_off", "shifter_service_test"),
    "w11416395-test-03b-motor.json": ("safety_power_off", "motor_wash_test"),
    "w11416395-test-04-hmi.json": ("safety_power_off", "hmi_service_test"),
    "w11416395-test-05-temp-thermistor.json": ("safety_power_off", "therm_sensor_feedback"),
    "w11416395-test-06-water-level.json": ("safety_power_off", "pressure_sensor_feedback"),
    "w11416395-test-08-lid-lock.json": ("safety_power_off", "lid_lock_service_test"),
}
...
def split_service_modes(attach_after: str, continue_to: str) -> list[dict[str, str]]:
    return [
        {
            "bundleId": "w11416395-service-diagnostic-entry",
            "modeKind": "service_diagnostic_entry",
            "attachAfterStepId": attach_after,
            "continueToStepId": "service_mode_before_test",
        },
        {
            "bundleId": "w11416395-service-test-mode",
            "modeKind": "load_test",
...
def main() -> None:
    for path in sorted(SEED_DIR.glob("w11416395-test-*.json")):
