#!/usr/bin/env python3
"""Apply SAMSUNG-TL-DV50-DRYER human gate from overlay mapping table v1."""
...
TABLE_PATH = (
    ...
    / "normalization"
    / "calibration"
    / "SAMSUNG_TL_DV50_DRYER_overlay_mapping_table_v1.json"
)
MANUAL_ID = "SAMSUNG-TL-DV50-DRYER"
REVIEWER = "human_gate"
...
def main() -> int:
    ...
    for cid, resolved in sorted(approve_map.items()):
        update_candidate_status(
            ...
            "approved",
            reviewer=REVIEWER,
            reason="SAMSUNG-TL-DV50-DRYER overlay mapping table v1",
            manual_id=MANUAL_ID,
    ...
    for cid in sorted(reject_ids):
        update_candidate_status(
            ...
            "rejected",
            reviewer=REVIEWER,
            reason="§4-1 / §4-4 procedure title — bind by procedureId only",
            manual_id=MANUAL_ID,
        )
        rejected_count += 1
