#!/usr/bin/env python3
"""Attach diagnosticEffects to W11366142 KitchenAid KDTM404 procedure branches."""
...
PROCEDURE_COMPONENT: dict[str, str] = {
    "w11366142-wash-motor-vsm": "circulation_pump",
    "w11366142-drain-motor": "drain_pump",
    "w11366142-dispenser": "inlet_valve",
    "w11366142-vent-wax-motor": "heater",
    "w11366142-tub-light": "heater",
    "w11366142-rif-filter": "circulation_pump",
}
...
def is_success_outcome(step: dict | None) -> bool:
    if not step or step.get("type") != "outcome":
        return False
    title = f"{step.get('title', '')} {step.get('oemOutcome', '')}".lower()
    return any(token in title for token in ("verified", "operates", "good", "pass", "retest", "within", "ok", "cleared", "resolved"))
...
def main() -> None:
    total = 0
    for path in sorted(SEED_DIR.glob("w11366142-*.json")):
