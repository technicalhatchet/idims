#!/usr/bin/env python3
"""Attach diagnosticEffects to Samsung ME11 OTR microwave procedure branches."""
...
PROCEDURE_COMPONENT: dict[str, str] = {
    "samsungotrmw-door-interlock": "door_interlock",
    "samsungotrmw-line-power": "line_fuse",
    "samsungotrmw-no-heat": "magnetron",
    "samsungotrmw-hv-transformer": "magnetron",
    "samsungotrmw-hv-capacitor": "hv_capacitor",
    "samsungotrmw-hv-diode": "magnetron",
    "samsungotrmw-magnetron": "magnetron",
    "samsungotrmw-vent-motor": "supply",
    "samsungotrmw-humidity-sensor": "supply",
    "samsungotrmw-temp-sensor": "thermal_cutout",
    "samsungotrmw-keypad-touch": "supply",
    "samsungotrmw-pcb-comm": "supply",
    "samsungotrmw-thermal-cutout": "thermal_cutout",
    "samsungotrmw-turntable": "magnetron",
}
...
def is_success_outcome(step: dict | None) -> bool:
    if not step or step.get("type") != "outcome":
        return False
    title = f"{step.get('title', '')} {step.get('oemOutcome', '')}".lower()
    return any(token in title for token in ("verified", "operates", "good", "pass", "retest", "within", "ok", "resolved", "addressed", "normal", "expected"))
...
def attach_branch_effects(step, branch, fallback_component, steps_by_id) -> bool:
    ...
    if step.get("type") == "visual_check":
        ...
        if kind == "checkpoint_no" and (is_failure_outcome(next_step) or branch.get("oemOutcome")):
            if confirm_id:
                branch["diagnosticEffects"] = [confirm_effect(component, confirm_id)]
                return True
        if kind == "checkpoint_yes" and (
            ...
            if eliminate_id:
                branch["diagnosticEffects"] = [eliminate_effect(component, eliminate_id)]
                return True
        if kind == "checkpoint_no" and not is_failure_outcome(next_step):
